...
const Home = ({ currentUser, onLogin, onLogout }) => {
  ...
  const loadHomeData = async () => {
    try {
      ...
      const guru = users.filter(u => u.role === 'guru' || u.role === 'walikelas');
        ...
        <section className="text-center mb-12">
          <div className="gradient-hero rounded-3xl p-12 text-white shadow-strong">
            ...
            <p className="text-lg opacity-75 max-w-2xl mx-auto">
              {schoolProfile?.visi || 'Sistem Informasi Akademik Digital untuk transparansi dan kemudahan akses informasi pendidikan'}
            </p>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {schoolProfile?.alamat || 'Jl. Pendidikan No. 1, Nosu, Kabupaten Mamuju Utara, Sulawesi Barat 91571'}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{schoolProfile?.telepon || '(0426) 123456'}</p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{schoolProfile?.email || 'info@sman1nosu.sch.id'}</p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{schoolProfile?.tahunBerdiri || '1985'}</p>
                </div>
              </div>
              ...
              {schoolProfile?.kepalaSekolah && (
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{schoolProfile.kepalaSekolah}</p>
                  </div>
                </div>
              )}
            </CardContent>
                ...
                {achievements.length > 0 ? achievements.map((achievement, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p className="text-sm text-foreground font-medium">{achievement.judul}</p>
                      <p className="text-xs text-muted-foreground">{achievement.tingkat} • {achievement.tahun}</p>
                    </div>
                  </div>
                )) : (
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {programs.length > 0 ? programs.map((program, index) => (
                  <div key={program.id || index} className="p-4 border border-border rounded-lg">
                    <h3 className="font-semibold text-foreground mb-2">{program.nama}</h3>
                    <p className="text-sm text-muted-foreground mb-3">{program.deskripsi}</p>
                    ...
                    {program.prospek && (
                      <div>
                        <p className="text-xs text-muted-foreground font-medium">Prospek Karir:</p>
                        <p className="text-xs text-muted-foreground">{program.prospek}</p>
                      </div>
                    )}
                  </div>
                )) : (
          ...
          <Card className="gradient-card shadow-medium">
            <CardContent className="p-8">
              <h2 className="text-2xl font-bold text-foreground mb-4">
                Bergabunglah dengan Keluarga Besar {schoolProfile?.nama || 'SMA Negeri 1 Nosu'}
              </h2>
              <p className="text-muted-foreground mb-6 max-w-2xl mx-auto">
                {schoolProfile?.misi || 'Wujudkan masa depan gemilang bersama pendidikan berkualitas, fasilitas modern, dan tenaga pengajar profesional.'}
              </p>
